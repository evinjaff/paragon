...
from paragon.model.fe14.dispo import Faction
...
from paragon.ui.fe14.actions.add_group_action import AddGroupAction
from paragon.ui.fe14.actions.add_spawn_action import AddSpawnAction
from paragon.ui.fe14.actions.delete_group_action import DeleteGroupAction
from paragon.ui.fe14.actions.delete_spawn_action import DeleteSpawnAction
from paragon.ui.fe14.actions.move_spawn_action import MoveSpawnAction
from paragon.ui.fe14.actions.paste_spawn_action import PasteSpawnAction
from paragon.ui.fe14.fe14_map_editor import FE14MapEditor
...
class FE14MapEditorDisposController:
    def __init__(self, map_editor: FE14MapEditor):
        ...
        self.view.coordinate_type_label.setText(
            "Coordinate Type: " + self.view.grid.coordinate_key
        )
    ...
    def _update_spawn_selection(self, spawn: Optional[PropertyContainer]):
        ...
        self.current_faction = (
            self.dispos_model.get_faction_from_spawn(spawn) if spawn else None
        )
    ...
    def _add_group(self):
        if self.active and self.dispos_model:
            (faction_name, ok) = QInputDialog.getText(
                self.view, "Enter a group name.", "Name:"
            )
            if ok:
    ...
    def update_active_spawn_position_from_grid_change(self, old_position, new_position):
        ...
        action = MoveSpawnAction(
            spawn, old_position, new_position, coordinate_key, self
        )
    ...
    def _undo(self):
        if (
            not self.active
            or not self.dispos_model
            or not self.dispos_model.undo_stack.can_undo()
        ):
            return
    ...
    def _redo(self):
        if (
            not self.active
            or not self.dispos_model
            or not self.dispos_model.undo_stack.can_redo()
        ):
            return
        undo_stack = self.dispos_model.undo_stack
        undo_stack.redo()
