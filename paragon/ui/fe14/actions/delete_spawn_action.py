class DeleteSpawnAction:
    ...
    def __repr__(self):
        return "<DeleteSpawnAction %s, %s>" % (
            self.spawn.get_display_name(),
            self.faction.name,
        )

    def undo(self):
        self.controller.dispos_model.add_spawn_to_faction(
            self.faction, spawn=self.spawn
        )
        spawn = self.faction.spawns[-1]
        self.controller.view.grid.add_spawn_to_map(spawn)
        self.controller.view.status_bar.showMessage(
            "Undo: Delete spawn " + self.spawn.get_display_name()
        )
    ...
    def redo(self):
        ...
        self.controller.view.status_bar.showMessage(
            "Redo: Delete spawn " + self.spawn.get_display_name()
        )
