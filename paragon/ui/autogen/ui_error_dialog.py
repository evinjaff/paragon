...
class Ui_error_dialog(object):
    def setupUi(self, error_dialog):
        ...
        QtCore.QObject.connect(
            self.buttonBox, QtCore.SIGNAL("accepted()"), error_dialog.accept
        )
        QtCore.QObject.connect(
            self.buttonBox, QtCore.SIGNAL("rejected()"), error_dialog.reject
        )
    ...
    def retranslateUi(self, error_dialog):
        error_dialog.setWindowTitle(
            QtWidgets.QApplication.translate(
                "error_dialog", "An error has occurred.", None, -1
            )
        )
        self.error_label.setText(
            QtWidgets.QApplication.translate("error_dialog", "TextLabel", None, -1)
        )
