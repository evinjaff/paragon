...
from paragon.model.fe14 import terrain, dispo
from paragon.model.fe14.chapter_data import ChapterData
from paragon.services.abstract_editor_service import AbstractEditorService
from paragon.services.service_locator import locator
from paragon.ui.fe14.fe14_chapter_editor import FE14ChapterEditor
from paragon.utils.chapter_utils import detect_chapter_file_sub_folder
...
class ChapterService(AbstractEditorService):
    ...
    def create_chapter(source, new_chapter_cid):
        ...
        config_archive = open_files_service.open_archive_direct(
            _CONFIG_PATH % source["CID"].value[4:]
        )
        person_archive = open_files_service.open_archive_direct(
            _PERSON_PATH % source_suffix
        )
        dispos_archive = open_files_service.open_archive_direct(
            _DISPOS_PATH % source_suffix
        )
        terrain_archive = open_files_service.open_archive_direct(
            _TERRAIN_PATH % source["CID"].value[4:]
        )
        text_archive = open_files_service.open_message_archive_direct(
            source_conversation_data_path
        )
        open_files_service.register_or_overwrite_archive(
            _CONFIG_PATH % new_chapter_cid[4:], config_archive
        )
        open_files_service.register_or_overwrite_archive(
            _PERSON_PATH % dest_suffix, person_archive
        )
        open_files_service.register_or_overwrite_archive(
            _DISPOS_PATH % dest_suffix, dispos_archive
        )
        open_files_service.register_or_overwrite_archive(
            _TERRAIN_PATH % new_chapter_cid[4:], terrain_archive
        )
        open_files_service.register_or_overwrite_message_archive(
            dest_conversation_data_path, text_archive
        )
