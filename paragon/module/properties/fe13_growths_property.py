...
def _handle_encode_decode(
    values: List[int], target_id: int, mode: int, encode_decode_pair
) -> List[int]:
    result = []
...
class FE13GrowthsProperty(BufferProperty):
    ...
    def read(self, reader):
        ...
        self.value = _handle_encode_decode(
            encrypted_values, target_id, self.mode, _DECODE_PAIR
        )

    def write(self, writer):
        module = self.parent.owner
        target_id = module.entries.index(self.parent)
        encrypted_values = _handle_encode_decode(
            self.value, target_id, self.mode, _ENCODE_PAIR
        )
